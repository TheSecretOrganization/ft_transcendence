...
class PongConsumerTest(ChannelsLiveServerTestCase):
    async def test_anonymous_user(self):
        ...
        """
        communicator = WebsocketCommunicator(PongGame.as_asgi(), "/ws/pong/?mode=local&player_needed=1")
    ...
    async def test_missing_parameters(self):
        ...
        communicator = WebsocketCommunicator(PongGame.as_asgi(), "/ws/pong/")
    ...
    async def test_websocket_connect(self):
        ...
        """
        user = await sync_to_async(get_user_model().objects.create_user)(username='testuser', password='password123')
        communicator = WebsocketCommunicator(PongGame.as_asgi(), "/ws/pong/?mode=local&player_needed=1&host=True&room_id=1")
        communicator.scope['user'] = user
